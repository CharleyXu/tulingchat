...
//        redisUtil.set("access_token", access_token, 2 * 60 * 60L);
	...
	public void crawlerArtist() {
		...
		System.out.println(artists.length + "---" + MusicUtil.InitialsList.size());
		...
		try {
			for (int i = 0, length = artists.length; i < length; i++) {
				...
				for (int j = 0, size = MusicUtil.InitialsList.size(); j < size; j++) {
					...
					String replaceUrl = stringBuilder.append(neteaseMusic).append(neteaseArtistCatUrl).toString().replace("ID", artist).replace("INITIAL", letter);
